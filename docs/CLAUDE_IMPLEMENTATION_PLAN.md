# Claude Implementation Plan — open GitHub issues #4–#17
...
against [CURRENT_STATE.md](CURRENT_STATE.md), [project-specs.md](project-specs.md),
[open-decisions.md](open-decisions.md) and the actual source under `src/`.

The previous contents of this file — the plan of the 2026-08 full-stack pass —
are kept verbatim in [§7 Appendix](#7-appendix-the-2026-08-full-stack-pass), which is
**historical** and must not be worked from. Outcomes of that pass are in
[CLAUDE_IMPLEMENTATION_REPORT.md](CLAUDE_IMPLEMENTATION_REPORT.md).
...
2. **The import now needs JavaScript.** It was a plain server-rendered form on
   ...
   an exception, and it is recorded in `CURRENT_STATE.md`'s "Not present" list.
...
[domain-model.md](domain-model.md#41-how-the-log-is-read-confirmed-2026-09-08--github-issue-16).
...
`CURRENT_STATE.md` routes/UI-maturity/verification rows, `TESTING.md` counts and
invariants, `DOCUMENT_MANIFEST.yaml`, and the link/anchor check, once behaviour
has stopped moving.
...
- **LaTeX math: already implemented.** `renderRichText` runs
  ...
  never does, by design** (project-specs.md §11) and that must not change.
...
- **Typst: not implemented, and two different features.** Rendering Typst
  ...
  import exists today (`docs/CURRENT_STATE.md`, `legacy-question-import.md`).
...
## 7. Appendix: the 2026-08 full-stack pass

> Everything below this line is the **verbatim** text of the 2026-08 plan, with
> its heading levels demoted so this file keeps one heading tree. Its baseline
> numbers, decision statuses and slice list are from its own date.

### Claude Implementation Plan — pilot-usable full-stack pass

> #### ⚠ HISTORICAL — a dated plan, not the current handoff
>
> **This is a record of one past implementation pass.** It is not the current
> implementation handoff: that is
> [CLAUDE_AUTONOMOUS_FULLSTACK_IMPLEMENTATION_PROMPT.md](../CLAUDE_AUTONOMOUS_FULLSTACK_IMPLEMENTATION_PROMPT.md),
> and what actually exists is [CURRENT_STATE.md](CURRENT_STATE.md). Its scope,
> decision statuses and test counts are all from its own date — do not plan
> against them.
>
> #### It also describes the removed account-matching flow
>
...
**Status:** **historical** — the working plan of a past implementation session, kept as a record. It is not the current plan and must not be worked from; the live handoff is [CLAUDE_AUTONOMOUS_FULLSTACK_IMPLEMENTATION_PROMPT.md](../CLAUDE_AUTONOMOUS_FULLSTACK_IMPLEMENTATION_PROMPT.md) and current behaviour is [CURRENT_STATE.md](CURRENT_STATE.md).
