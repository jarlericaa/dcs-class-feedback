...
[public-qa-and-source-linking.md](public-qa-and-source-linking.md) §3 and backs
Risk R2:
...
this ([PILOT-STRATEGY.md](PILOT-STRATEGY.md) §2). The variance that matters here
...
pass to end ([INTENT-CONTEXT.md](INTENT-CONTEXT.md) §1.2). A teacher in a hurry
...
- The original student message is displayed read-only in its own block
  ...
  exactly what [UX-DESIGN-BRIEF.md](UX-DESIGN-BRIEF.md) asks for.
...
([public-qa-and-source-linking.md](public-qa-and-source-linking.md) §3: *"Rewording
...
[SECURITY.md](SECURITY.md) lists as a hard invariant that the original wording
...
Merge UI does not exist ([CURRENT_STATE.md](CURRENT_STATE.md)), so every publish
...
(F2 in [JOURNEY-STUDENT-SUBMIT.md](JOURNEY-STUDENT-SUBMIT.md)) is the same class
...
not hidden — [UX-DESIGN-BRIEF.md](UX-DESIGN-BRIEF.md) forbids burying important
...
| Flag region | `aria-live="polite"`, associated with the field via `aria-describedby`. A flag a screen-reader user does not hear is a privacy failure, not a polish issue ([INTENT-CONTEXT.md](INTENT-CONTEXT.md) §4.2). |
...
learning is **[Open]** ([PILOT-STRATEGY.md](PILOT-STRATEGY.md) §6 item 5).
...
- **Not designing here:** merge UI (out of scope, and
  [PILOT-STRATEGY.md](PILOT-STRATEGY.md) §4 Q4 questions whether it is needed),
...
[roles-and-permissions.md](roles-and-permissions.md) · [SECURITY.md](SECURITY.md) ·
