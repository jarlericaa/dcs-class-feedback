...
nothing from [product/scope.md](../product/scope.md) §2/§3 is promoted, and no new entity is
proposed.
...
> rationale: [domain/forms-and-audiences.md](../domain/forms-and-audiences.md).
>
...
> [design/journeys/teacher-setup.md](journeys/teacher-setup.md) §5 proposed reordering the
...
3. **Question backlog sits in the section group**, but the backlog is
   ...
   *"Backlog is course-level and separate from the weekly dashboard."*
   [engineering/current-state.md](../engineering/current-state.md) confirms the route only exists under a
...
[design/journeys/teacher-setup.md](journeys/teacher-setup.md) proposal was to lead with
...
[design/journeys/teacher-setup.md](journeys/teacher-setup.md) §5–6. That is the correct
...
[domain/public-qa.md](../domain/public-qa.md) §8. "Archive"
...
The docstring attributes the IA to a "Reference translation" of the Ed
Discussion study ([research.md](research.md)) — so the eight-member vocabulary
is that product's category model, not this one's.

**Why it matters.** The confirmed domain taxonomy is three categories
([domain/form-workflow.md](../domain/form-workflow.md) §7: Content / Logistics /
...
**Evidence.** [domain/public-qa.md](../domain/public-qa.md)
...
drawer on phones ([engineering/current-state.md](../engineering/current-state.md)). With three groups the
...
([domain/form-workflow.md](../domain/form-workflow.md) §7). MECE holds reasonably:
...
identified in [design/journeys/student-submit.md](journeys/student-submit.md) §2:
...
| **Can a cold-start teacher find the roster importer?** | First-click test on the new-section nav: *"You need to add your students"* | Same participants | F1's core claim. Folds into [product/pilot-strategy.md](../product/pilot-strategy.md) §5 activity #3 — no extra session needed. |
...
[product/pilot-strategy.md](../product/pilot-strategy.md) already proposes.** That is deliberate: IA
...
1. **The course-level backlog route does not exist.** The domain model says the
   ...
   ([engineering/current-state.md](../engineering/current-state.md)). Building
...
2. **Topic authoring has no UI.** Exposing topic as a browse facet (F5) assumes
   ...
   ([domain/form-workflow.md](../domain/form-workflow.md) §6), but no authoring
   surface exists. **Sequencing: the facet cannot ship before the authoring
   does.** Worth confirming against
   [product/pilot-strategy.md](../product/pilot-strategy.md) §4 Q4 — this may be a feature nobody
...
- `/wireframe` — where the three nav groups sit, drawer behaviour on phones with
  ...
  vocabulary (F4).
- `/journey` — [design/journeys/teacher-setup.md](journeys/teacher-setup.md) §5 should be
...
[design/journeys/teacher-setup.md](journeys/teacher-setup.md) ·
[design/journeys/student-submit.md](journeys/student-submit.md) ·
[design/journeys/student-first-run.md](journeys/student-first-run.md) ·
[design/journeys/teacher-publish.md](journeys/teacher-publish.md) ·
...
[domain/question-backlog.md](../domain/question-backlog.md) ·
[context.md](context.md) · [product/pilot-strategy.md](../product/pilot-strategy.md) ·
[engineering/current-state.md](../engineering/current-state.md)
