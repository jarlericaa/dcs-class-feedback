...
> This document **owns** the import of historical questions and its privacy defaults. Backlog behavior: [domain/question-backlog.md](question-backlog.md). Roster CSV import is separate — see [domain/student-identity.md](student-identity.md#7-class-list-import-confirmed-project-specsmd-61).
...
These map to `BacklogQuestion` fields: source provenance, identity-preservation flag, optional source link, answered/unanswered marker ([domain/domain-model.md](domain-model.md)).

## 5. Import methods **[Confirmed — product/specification.md §8 P1, approved 2026-08-03]**
...
- Each import creates an `ImportBatch` (kind = legacy) recording source kind, file name, source
  ...
  ([domain/domain-model.md](domain-model.md#audit-events)).
...
([participation.md](participation.md#3-participation-derivation)). This is structural,
...
When a legacy question is later published, source-link and anonymity rules from [public-qa.md](public-qa.md) apply. A legacy entry deliberately kept anonymous publishes with **no** source link (a valid case of a `Published` answer with zero source links — see [domain/domain-model.md](domain-model.md#39-invalid-combinations-illustrative)).

## 9. Related documents

[domain/question-backlog.md](question-backlog.md) · [domain/domain-model.md](domain-model.md) · [domain/student-identity.md](student-identity.md) · [public-qa.md](public-qa.md) · [participation.md](participation.md) · [product/scope.md](../product/scope.md)
