...
> [forms-and-audiences.md](forms-and-audiences.md)** —
> this document links there rather than restating them. State definitions live in
> [domain/domain-model.md](domain-model.md#3-state-models); scheduling infrastructure in
> [engineering/architecture-history.md](../engineering/architecture-history.md#scheduling).
...
([D18/D19](../decisions/open-decisions.md)). Weekly is one of four delivery modes, not the
...
rather than silently resolved; per-section windows are [D22](../decisions/open-decisions.md).

Timezone: **[Confirmed]** institution timezone, stored on each section —
**D7** closed 2026-08-03, with per-section overrides deferred
([decisions/open-decisions.md](../decisions/open-decisions.md)). All open/deadline/schedule times are
...
- **[Confirmed]** Scheduled instances open automatically at their opening time
  (`Scheduled → Open`, [domain/domain-model.md](domain-model.md#31-form-instance-state)).
...
- Duplicate-prevention and reconciliation are shared infrastructure with scheduled publication — see [engineering/architecture-history.md](../engineering/architecture-history.md#scheduling).
...
- **Per-occurrence customization** — varying one week's questions without touching
  ...
  [domain/forms-and-audiences.md §5](forms-and-audiences.md).
...
- The response's **attribution section** is recorded separately and is
  ...
  ([domain/forms-and-audiences.md §2.4](forms-and-audiences.md)).
- **A student may save a draft, submit, and then edit that same response until the deadline**
  (`product/specification.md` §4.3, §6.3, §7 B3). At the deadline the latest submitted version locks.
...
- Submitting a complete form counts as participation unless later marked invalid — see [participation.md](participation.md).

### 4.1 Student-originated items **[Confirmed — product/specification.md §5.2, §5.5, §7 D1]**
...
- **[Recommended]** Options carry a **stable internal id** separate from the display label, so exports and analytics stay stable when labels are edited. Answers store the chosen option ids *and* the labels at submission time (see [participation.md](participation.md#detailed-response-export)).
...
[domain/domain-model.md](domain-model.md)). "Template" is the historical table name; the
...
- The **original text is immutable** — see rewording rules in [public-qa.md](public-qa.md).
...
[forms-and-audiences.md](forms-and-audiences.md) · [domain/domain-model.md](domain-model.md) · [participation.md](participation.md) · [public-qa.md](public-qa.md) · [engineering/architecture-history.md](../engineering/architecture-history.md) · [decisions/open-decisions.md](../decisions/open-decisions.md)
