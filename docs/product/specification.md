...
- ~~Automatic account-name match threshold and normalization rules~~ — **closed by decision D23 (2026-08-07): there is no name matching.** Identity is exact normalized UP-email equality against the class list; the only normalization is trim + lowercase, applied to both sides. See [domain/student-identity.md](../domain/student-identity.md) and story A4.
