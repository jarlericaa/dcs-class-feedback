...
- **Review, private & public responses** — a course-wide review workspace with an optional section filter; private responses; anonymous public answers owned by the **course**, published once and read by every section of it ([ADR-0005](../decisions/ADR-0005-course-scoped-teaching-workflow.md)); rewording (original never destroyed); merge; scheduled publication; source links. See [domain/public-qa.md](../domain/public-qa.md).
- **Course-level backlog** — separate from the weekly dashboard; answering an item publishes **one** course-wide entry, with no target section to choose. See [domain/question-backlog.md](../domain/question-backlog.md).
