...
> ### Status-symbol decision recorded 2026-09-15
>
> | # | Question | Decision |
> |---|---|---|
> | S-1 | Should status stamps keep the square, triangle, diamond and hollow-square marks? | **No — remove status symbols everywhere.** Published, Scheduled, Needs reply and every other `Stamp` render their written label only, with tone retained as reinforcement. `Stamp` no longer has a symbol opt-out because text-only is the universal rule. Category silhouettes remain because they identify question metadata rather than status. |
