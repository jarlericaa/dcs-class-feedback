...
approved and implemented ([decisions/open-decisions.md](../decisions/open-decisions.md),
...
**Explicitly avoided** (per [product/scope.md](../product/scope.md)): microservices, message brokers, separate databases, event-driven infrastructure, dedicated vector databases, standalone AI services.
...
|--------|----------------|----------|
| **identity** | SSO, users, roster, email-based student resolution, enrollment | [domain/student-identity.md](../domain/student-identity.md) |
| **catalog** | courses, sections, staff, permissions, lessons/topics | [domain/roles-and-permissions.md](../domain/roles-and-permissions.md) |
| **forms** | templates, recurrence, cycles, questions, responses | [domain/form-workflow.md](../domain/form-workflow.md) |
| **review & publishing** | dashboard, private/public responses, rewording, merge, scheduling, archive | [domain/public-qa.md](../domain/public-qa.md) |
| **backlog & import** | course backlog, legacy import | [domain/question-backlog.md](../domain/question-backlog.md), [domain/legacy-question-import.md](../domain/legacy-question-import.md) |
| **participation & export** | derived participation, CSV exports | [domain/participation.md](../domain/participation.md) |
| **audit** | append-only audit events | [domain/domain-model.md](../domain/domain-model.md#audit-events) |
...
| **F# + Fable / SAFE-stack** | Would have applied had D1 resolved to F#/Fable | Steeper learning curve; smaller hiring/help pool | **Not selected** — D1 closed 2026-08-03 on the TypeScript baseline ([decisions/open-decisions.md](../decisions/open-decisions.md)) |

<a id="scheduling"></a>

## 4. Scheduling design **[Implemented]**

Two scheduled concerns share one DB-backed mechanism: **weekly-cycle open/close** ([domain/form-workflow.md](../domain/form-workflow.md#2-instance-generation--auto-open)) and **public-answer publication** ([domain/public-qa.md](../domain/public-qa.md#7-scheduled-public-answers)).
...
Implements the deny-by-default, resource-scoped model in [domain/roles-and-permissions.md](../domain/roles-and-permissions.md):
...
- An **append-only** `AuditEvent` store capturing actor, action, timestamp, affected entity, and before/after values ([domain/domain-model.md](../domain/domain-model.md#audit-events)).
...
- **Import** (roster + legacy): validate → map columns → detect duplicates → preview → per-row errors → summary → audited `ImportBatch`; never silently overwrite ([domain/student-identity.md](../domain/student-identity.md#7-class-list-import-confirmed-project-specsmd-61), [domain/legacy-question-import.md](../domain/legacy-question-import.md)).
- **Export** (participation): the three reports in [domain/participation.md](../domain/participation.md#4-participation-csv-exports); identity-bearing, staff-only, access audited; emit stable ids alongside labels.
...
- **Deployment options:** a single container + managed Postgres (simplest); or a PaaS that runs the Next.js app with a managed Postgres add-on. Choice deferred; both keep the monolith simple. This is not decided ([Open D12](../decisions/open-decisions.md)).
...
suite — [current-state.md](current-state.md) records the E2E suite as `missing`,
...
[domain/domain-model.md](../domain/domain-model.md) · [domain/roles-and-permissions.md](../domain/roles-and-permissions.md) · [domain/form-workflow.md](../domain/form-workflow.md) · [domain/public-qa.md](../domain/public-qa.md) · [domain/participation.md](../domain/participation.md) · [product/ai-future-plan.md](../product/ai-future-plan.md) · [decisions/open-decisions.md](../decisions/open-decisions.md)
