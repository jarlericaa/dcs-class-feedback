...
This is a factual snapshot. Scope is in [product/scope.md](../product/scope.md); resolved
decisions are in [decisions/open-decisions.md](../decisions/open-decisions.md).
...
is the live design authority** — it wins over every other design document. The
UI documents written before 2026-08-07 were removed rather than kept as history:
they specified the account-matching era's `/claim` and
`/teach/sections/[id]/matches`, neither of which exists.

## Feature status against product/specification.md
...
| A4 | Deterministic UP-email student access | **complete** — a signed-in account resolves to the class-list row whose UP email equals its normalized email; no claim page, no confirmation step. Roster claims and account matching were **removed** 2026-08-07 ([domain/student-identity.md](../domain/student-identity.md)) |
...
| `/teach/courses/[id]/publications` | The course's **one** publication queue (ADR-0005): drafts, scheduled answers, failed publications, retry/reschedule/cancel. A recently published question links to its entry in Class Q&A (issue #13). No section target is asked for — publishing reaches the whole course |
| `/teach/courses/[id]/backlog` | The course's **one** question backlog: triage, drafting (one course-wide answer per item), anonymous legacy paste-import |
...
3. `0003` adds course-level forms, shared audiences and generalized form
   ...
   [domain/forms-and-audiences.md](../domain/forms-and-audiences.md).
4. `0004` adds the deterministic UP-email identity column, hand-edited to carry
   its backfill ([domain/student-identity.md](../domain/student-identity.md)).
