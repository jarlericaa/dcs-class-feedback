...
export function WeeklyForm({
  ...
  const [values, setValues] = useState<Record<string, AnswerValue>>(initialAnswers);
  ...
  const [items, setItems] = useState<StudentItemView[]>(() => {
    ...
    const comment = initialItems.find((item) => item.kind === "general_comment");
    return [
      ...(existing.length > 0
        ? existing
    ...
    <form action={formAction} noValidate>
        ...
        return (
          <fieldset className="question-block" key={question.id}>
            ...
            {(question.descriptionHtml || question.description) && (
              <div className="question-description" id={`desc-${question.id}`}>
      ...
      {config.maxStudentQuestions > 0 && (
        <fieldset className="student-item">
          <legend>
            {config.studentQuestionPrompt ?? "Your own questions (optional)"}
          </legend>
          <p>
            Each question is tracked separately, so staff can answer them
            independently. They can reply privately, or reword a question and
            publish the answer to this class — your classmates never see your name
              ...
              >
                ...
                {refused && (
                  <Alert variant="warning">
                    This could not be changed because staff have already acted on
                    it. Your original wording stands.
                  </Alert>
                )}
      ...
      <div className="form-actions">
        <p className="muted small" style={{ margin: 0 }}>
          {locked ? (
            ...
            <>
              Submitted{lastEditedLabel ? ` · last edited ${lastEditedLabel}` : ""}
